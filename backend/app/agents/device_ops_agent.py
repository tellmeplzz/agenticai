...
class DeviceOpsAgent(BaseAgent):
    ...
    def handle_message(
        ...
        knowledge_base = self._device_ops_service.get_standard_operating_procedures()
        maintenance_history = self._device_ops_service.get_maintenance_history(
            device_id=telemetry.get("device_id") if isinstance(telemetry, dict) else None,
            limit=10,
        )
